...
import { inrShort } from '../utils/helpers'
  ...
  return (
    <ResponsiveContainer width="100%" height={250}>
      <AreaChart data={data} margin={{ top: 10, right: 8, left: 0, bottom: 0 }}>
        ...
        <Tooltip contentStyle={tipStyle} formatter={(v: any) => [inrShort(Number(v)), 'Sales']} cursor={{ stroke: '#e3a92e', strokeWidth: 1 }} />
  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <BarChart data={data} margin={{ top: 10, right: 8, left: 0, bottom: 0 }} barGap={4}>
        ...
        <YAxis tick={AXIS} axisLine={false} tickLine={false} tickFormatter={inrShort} width={56} />
        <Tooltip contentStyle={tipStyle} formatter={(v: any, n: any) => [inrShort(Number(v)), n === 'sales' ? 'Sales' : 'Purchase']} cursor={{ fill: 'rgba(16,35,27,.04)' }} />
...
export function BrandPie({ data }: { data: any[] }) {
  return (
    <ResponsiveContainer width="100%" height={230}>
      <PieChart>
        ...
        <Tooltip contentStyle={tipStyle} formatter={(v: any, n: any) => [inrShort(Number(v)), n]} />
      </PieChart>
    </ResponsiveContainer>
  )
}
  ...
  return (
    <ResponsiveContainer width="100%" height={180}>
      <PieChart>
        ...
        <Tooltip contentStyle={tipStyle} formatter={(v: any, n: any) => [`${v} (${Math.round((v as number) / total * 100)}%)`, n]} />
      </PieChart>
    </ResponsiveContainer>
  )
}
