...
import { BrandPie, MonthlyBarChart, PIE_COLORS } from '../components/Charts.jsx'
import { inr, inrShort, num, csvExport, orderTotals, orderProfit } from '../utils/helpers.js'

const REPORTS = [
  { key: 'sales',      label: 'Sales Report',          icon: 'sales',     desc: 'Order-wise revenue, GST & profit' },
  ...
  { key: 'customer',   label: 'Customer Outstanding',  icon: 'customers', desc: 'Dealer dues & credit' },
  { key: 'supplier',   label: 'Supplier Outstanding',  icon: 'suppliers', desc: 'Payable to vendors' },
  ...
  { key: 'gst',        label: 'GST Report',            icon: 'doc',       desc: 'Output tax summary' },
]
...
export default function Reports() {
  ...
  const brandPie = useMemo(() => Object.entries(metrics.brandSales).map(([name, value]) => ({ name, value })).sort((a, b) => b.value - a.value), [metrics.brandSales])

  const data = useMemo(() => {
    switch (active) {
      case 'sales':
        return {
          cols: ['Order', 'Customer', 'Date', 'Total', 'GST', 'Profit', 'Status'],
          rows: sales.map((s) => {
            const t = orderTotals(s.items, productMap)
            return [s.id, customerMap[s.customerId]?.name, s.date, inr(t.total), inr(t.tax), inr(orderProfit(s.items, productMap)), s.payStatus]
          }),
        }
      case 'brand':
        return { cols: ['Brand', 'Revenue', 'Share'], rows: brandPie.map((b) => [b.name, inr(b.value), `${Math.round(b.value / (metrics.totalRevenue || 1) * 100)}%`]) }
      case 'product': {
        const list = Object.entries(metrics.prodSales).map(([id, v]) => ({ p: productMap[id], v })).filter((x) => x.p).sort((a, b) => b.v - a.v)
        return { cols: ['Product', 'Brand', 'Revenue'], rows: list.map((x) => [x.p.name, brandMap[x.p.brandId]?.name, inr(x.v)]) }
      }
      case 'customer':
        return { cols: ['Customer', 'Type', 'Credit Limit', 'Outstanding'], rows: customers.map((c) => [c.name, c.type, inr(c.creditLimit), inr(c.outstanding)]) }
      case 'supplier':
        return { cols: ['Supplier', 'Material', 'Rating', 'Due'], rows: suppliers.map((s) => [s.name, s.material, `${s.rating}/5`, inr(s.due)]) }
      case 'stock':
        return { cols: ['Product', 'Brand', 'Stock', 'Min', 'Status'], rows: products.map((p) => [p.name, brandMap[p.brandId]?.name, num(p.stock), num(p.minStock), p.stock <= p.minStock ? 'LOW' : 'OK']) }
      case 'production':
        return { cols: ['Batch', 'Product', 'Planned', 'Output', 'Status'], rows: production.map((b) => [b.id, productMap[b.productId]?.name, num(b.plannedQty), num(b.outputQty), b.status]) }
      case 'gst':
        return {
          cols: ['Invoice', 'Customer', 'Taxable', 'GST', 'Total'],
          rows: sales.map((s) => { const t = orderTotals(s.items, productMap); return [`INV-${s.id.replace('SO-', '')}`, customerMap[s.customerId]?.name, inr(t.sub), inr(t.tax), inr(t.total)] }),
        }
      default: return { cols: [], rows: [] }
    }
  }, [active, sales, customers, suppliers, products, production, productMap, customerMap, brandMap, brandPie, metrics])
  ...
  return (
    <div className="page">
      <PageHeader title="Reports & Analytics" subtitle="Drill into sales, stock, production, GST and outstanding reports.">
        <button className="btn btn-gold" onClick={exportCurrent}><Icon name="download" /> Export current</button>
      </PageHeader>
